    ...
    <section id="faq" className="py-16 md:py-20 kf-section-card border-t">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20">
        ...
        <div className="mt-4 flex flex-col md:flex-row md:justify-between md:items-start gap-6">
          ...
          <div className="border-b-2 border-slate-200 pb-2 flex items-center md:mt-0 mt-4 md:w-auto w-full">
            <input
              ...
              className="lg:w-96 md:w-72 w-full focus:outline-none placeholder-slate-500 text-base text-slate-700 leading-4 bg-transparent"
            />
            <Search className="shrink-0 w-4 h-4 text-slate-600" aria-hidden />
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:space-x-8 md:mt-16 mt-8 gap-10 md:gap-0">
          ...
          <div className="md:w-7/12 lg:w-8/12 w-full md:mt-0">
            {filtered.length === 0 ? (
              <p className="text-slate-500 text-sm py-8">{t.faqNoResults}</p>
            ) : (
                ...
                return (
                  <div key={item.id}>
                    <div className="flex justify-between items-center">
                      <h3 className="font-semibold text-xl leading-5 text-slate-800">
                      ...
                      <button
                        ...
                        className="text-slate-800 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-800 shrink-0 ml-4 p-1"
                      >
                    ...
                    <p
                      className={`font-normal text-base leading-6 text-slate-600 mt-4 w-11/12 ${
                        isOpen ? 'block' : 'hidden'
                      }`}
                    >
                      {item.answerBn || item.answer}
                    </p>
                    {idx < filtered.length - 1 && <hr className="my-7 bg-slate-200 border-0 h-px" />}
                  </div>
                );
              })
            )}
